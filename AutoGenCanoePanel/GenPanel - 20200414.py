...
class GEN_PANEL(wx.Frame):
    def __init__(self, dbcFile, panelName):   
        ...
        self.creat_PanelFile(panelName)
    ...
    def creat_SignalLibrary(self, nodeName, messageName, signalArray):
        ...
        signalInfoDir["SIG_ATTR"][self.AttrNameList[VIEW_COL_NUM]] = '1'
        signalInfoDir["SIG_ATTR"][self.AttrNameList[CONTROL_COL_NUM]] = '1'
        signalInfoDir["SIG_ATTR"][self.AttrNameList[LOCK_TRIGGER_COL_NUM]] = '1'
        signalInfoDir["SIG_ATTR"][self.AttrNameList[UNLOCK_TRIGGER_COL_NUM]] = '1'
        signalInfoDir["SIG_ATTR"]["DISPLAY"] = {}
        signalInfoDir["SIG_ATTR"]["DISPLAY"][self.AttrNameList[VIEW_COL_NUM]] = True
        signalInfoDir["SIG_ATTR"]["DISPLAY"][self.AttrNameList[CONTROL_COL_NUM]] = True
    ...
    def creat_PanelFile(self, panelName):
        ...
        attributeList  =[("Type", "Vector.CANalyzer.Panels.Runtime.Panel, Vector.CANalyzer.Panels.Common, Version=9.0.86.0, Culture=neutral, PublicKeyToken=null"), ("Name", panelName), ("ControlName", panelName)]
        ...
        file_object = open(ThisFilePath + '/TestPanel/Panel/' + panelName + ".xvp", "w")  
                ...
                for signal in self.NodeInfoArrayDict[node]["NODE_TX_MSGS"][message]["MSG_SIGS"].keys():
                    for i in range(0, len(self.AttrNameList)):
                        if("1" == self.NodeInfoArrayDict[node]["NODE_TX_MSGS"][message]["MSG_SIGS"][signal]["SIG_ATTR"][self.AttrNameList[i]]):
                            if(node not in list(nodeInfoArrayDictUsed.keys())):
    ...
    def get_needDisplaiedSignalArrayDict(self, messageInfoArrayDir, message, attr):
        signalInfoArrayDir = {}
        for signal in messageInfoArrayDir[message]["MSG_SIGS"]:
    ...
    def get_ifAllSignalDisplaied(self, messageInfoArrayDir, message):
        result = True
        for signal in messageInfoArrayDir[message]["MSG_SIGS"]:
            if((True == messageInfoArrayDir[message]["MSG_SIGS"][signal]["SIG_ATTR"]["DISPLAY"][self.AttrNameList[CONTROL_COL_NUM]]) or (True == messageInfoArrayDir[message]["MSG_SIGS"][signal]["SIG_ATTR"]["DISPLAY"][self.AttrNameList[CONTROL_COL_NUM]])):
                result = False
                return(result)
    ...
    def creat_PanelLayout(self, parent):
        ...
        while({} != nodeInfoArrayDictCopy):
            ...
            # print("+++" + node + "  " + str(nodeInfoArrayDictCopy[node]["NODE_LAYOUT"]["TOTALROWSIZE"]))
            if(0 == (tabUsedCnt % 2)): 
            ...
            while(heightUsedValue <= heightLimitValue and 0 < len(messageInfoArrayDirCopy)):
                ...
                # print(message + ":  " + str(heightUsedValue) + ", " + str(heightLimitValue) + "   (" + str(nodeGroupBoxPosX) + ", " + str(nodeGroupBoxPosY) + ")")   
                ...
                signalInfoArrayDirControlCopy = self.get_needDisplaiedSignalArrayDict(messageInfoArrayDirCopy, message, CONTROL_COL_NUM)
                signalInfoArrayDirViewCopy = self.get_needDisplaiedSignalArrayDict(messageInfoArrayDirCopy, message, VIEW_COL_NUM)
                if({} != signalInfoArrayDirControlCopy):
                    if(((nodeGroupBoxPosY + messageGroupBoxPosY + 2*(PANEL_ELEMENT_ROW_SIZE + PANEL_ELEMENT_GAP_SIZE)) < TAB_HEIGTH_SIZE) and 0 < len(signalInfoArrayDirControlCopy)):
                        ...
                        while((nodeGroupBoxPosY + messageGroupBoxPosY + signalGroupBoxControlY + PANEL_ELEMENT_ROW_SIZE) <= TAB_HEIGTH_SIZE and 0 < len(signalInfoArrayDirControlCopy)):
                            ...
                            if(False == signalInfoArrayDirControlCopy[signal]["SIG_ATTR"]["DISPLAY"][self.AttrNameList[CONTROL_COL_NUM]]):
                                # pass
                                ...
                                if({} == signalInfoArrayDirControlCopy):
                                    print("haha")
                                    print(signalInfoArrayDirViewCopy)
                                # signalInfoArrayDirControlCopy.pop(signal)
                            messageInfoArrayDirCopy[message]["MSG_SIGS"] = deepcopy(signalInfoArrayDirControlCopy)
                ...
                elif({} == signalInfoArrayDirControlCopy and {} != signalInfoArrayDirViewCopy):
                    print(signalInfoArrayDirViewCopy)
                    if(((nodeGroupBoxPosY + messageGroupBoxPosY + 2*(PANEL_ELEMENT_ROW_SIZE + PANEL_ELEMENT_GAP_SIZE)) < TAB_HEIGTH_SIZE) and 0 < len(signalInfoArrayDirViewCopy)):
                        ...
                        print(signalInfoArrayDirViewCopy)
                        while((nodeGroupBoxPosY + messageGroupBoxPosY + signalGroupBoxControlY + PANEL_ELEMENT_ROW_SIZE) <= TAB_HEIGTH_SIZE and 0 < len(signalInfoArrayDirViewCopy)):
                            ...
                            if('1' == messageInfoArrayDirCopy[message]["MSG_SIGS"][signal]["SIG_ATTR"][self.AttrNameList[VIEW_COL_NUM]]):
                                try:
                                    if('' != messageInfoArrayDirCopy[message]["MSG_SIGS"][signal]["SIG_ATTR"]["VALUETABLE"]):
                                        typeTxt = commonControlTypeTxt.replace('ReplaceXXX', 'ComboBoxControl')
                                        attributeList  =[("Type", typeTxt), ("Name", 'ComboBoxControl' + str(elementNumberSignal)), ("Children", "Controls"), ("ControlName", "")]
                                        xlmNode_SignalComboBoxControl1 = self.xlm_NodeAdd(xlmNode_MessageSubGroupBoxView, "Object", attributeList, '')
                                        symbolConfigurationValue = "4;16;SIGNALINFO;;;" + signal + "Value;1;;;-1"   
                                        self.xlm_NodeAdd(xlmNode_SignalComboBoxControl1, "Property", [("Name","Size")], '300, 20')
                                        self.xlm_NodeAdd(xlmNode_SignalComboBoxControl1, "Property", [("Name","Location")],  '10 , ' + str(signalGroupBoxControlY))
                                        self.xlm_NodeAdd(xlmNode_SignalComboBoxControl1, "Property", [("Name","DropDownWidth")], '115')  
                            ...
                            if(False == signalInfoArrayDirViewCopy[signal]["SIG_ATTR"]["DISPLAY"][self.AttrNameList[VIEW_COL_NUM]]):
                                signalInfoArrayDirViewCopy.pop(signal)
                            messageInfoArrayDirCopy[message]["MSG_SIGS"] = deepcopy(signalInfoArrayDirViewCopy)
                            
                        signalGroupBoxControlY = signalGroupBoxControlY + PANEL_ELEMENT_GAP_SIZE        
                        self.xlm_NodeAdd(xlmNode_MessageSubGroupBoxView, "Property", [("Name","Name")], 'MessageControl' + message.replace('_','') + str(elementNumberMessage))
                        self.xlm_NodeAdd(xlmNode_MessageSubGroupBoxView, "Property", [("Name","Size")], '340, ' + str(signalGroupBoxControlY))
                ...
                messageSize = messageSize + messageGroupBoxPosY + signalGroupBoxControlY
                ...
                '''
                if(0 >= len(messageInfoArrayDirCopy[message]["MSG_SIGS"])):
                    messageInfoArrayDirCopy.pop(message)
                '''    
                nodeInfoArrayDictCopy[node]["NODE_TX_MSGS"] = deepcopy(messageInfoArrayDirCopy)  
                # print(message + ":  " + str(heightUsedValue) + ", " + str(heightLimitValue) + "   (" + str(nodeGroupBoxPosX) + ", " + str(nodeGroupBoxPosY) + ")   " + str(messageSize))  
                # print("*******************\n\n")
            ...
            for message in self.NodeInfoArrayDict[node]["NODE_TX_MSGS"].keys():
                ...
                for signal in self.NodeInfoArrayDict[node]["NODE_TX_MSGS"][message]["MSG_SIGS"].keys():
                    ...
                    if("SUM" in signal.upper()):
                        caplFile.write("  }\n")
                try:
                    caplFile.write("  setTimer(" + message + "_Timer, " + str(self.NodeInfoArrayDict[node]["NODE_TX_MSGS"][message]["MSG_ATTR"]["CYCLE"]) + ");\n")
                except:
                    caplFile.write("  setTimer(" + message + "_Timer, " + str(0) + ");\n")
...
def main():
    app = wx.PySimpleApp()
    Test = GEN_PANEL(r"DBC_SAIC_SK83_V1.2.dbc", "SK83")
    Test.frameShow()
    app.MainLoop()
